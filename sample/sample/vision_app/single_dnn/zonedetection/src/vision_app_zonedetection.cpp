...
/* static                                                   */
/* -------------------------------------------------------- */
static PPL_CustomParam s_custom_param = {0};//{PPL_DNN_OUTPUT_DETECTIONS,PPL_DNN_OUTPUT_DETECTIONS, PPL_DEFAULT_MODE, PPL_DEFAULT_THRESHOLD, PPL_CUSTOM_INPUT_TENSOR_WIDTH, PPL_CUSTOM_INPUT_TENSOR_HEIGHT}; //Set Max detections=10 Default Threshold= 0.3
...
int main(int argc, char *argv[]) {
    ...
    while (!s_is_evp_exit) {
        ...
        ret = PPL_ObjectDetectionSsdAnalyze((float *)raw_data.address, raw_data.size/4, &p_out_buf, &out_size, s_custom_param);
...
static void ConfigurationCallback(const char *topic, const void *config, size_t configlen, void *userData) {
    ...
    if (strcmp((char *)config, "") == 0) {
        ...
        INFO_PRINTF("dnn_output_detections: %d  max_detections: %d mode: %d input_width: %d input_height: %d", \
    ...
    EPPL_RESULT_CODE ret = json_parse(root_value, &s_custom_param);
    if (ret != E_PPL_OK) {

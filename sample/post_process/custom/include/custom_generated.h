...
#ifndef FLATBUFFERS_GENERATED_CUSTOM_SMARTCAMERA_H_
#define FLATBUFFERS_GENERATED_CUSTOM_SMARTCAMERA_H_
...
struct GeneralObject FLATBUFFERS_FINAL_CLASS : private flatbuffers::Table {
  ...
  uint32_t zoneflag() const {
    return GetField<uint32_t>(VT_ZONEFLAG, 0);
  }
  bool Verify(flatbuffers::Verifier &verifier) const {
    return VerifyTableStart(verifier) &&
           ...
           VerifyField<float>(verifier, VT_IOU) &&
           VerifyField<uint32_t>(verifier, VT_ZONEFLAG) &&
           verifier.EndTable();
  }
};
...
struct GeneralObjectBuilder {
  ...
  void add_zoneflag(uint32_t zoneflag) {
    fbb_.AddElement<uint32_t>(GeneralObject::VT_ZONEFLAG, zoneflag, 0);
  }
...
#endif  // FLATBUFFERS_GENERATED_CUSTOM_SMARTCAMERA_H_
